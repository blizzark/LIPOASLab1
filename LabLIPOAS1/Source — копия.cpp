...
string FileEnter() {
	...
	cout << "Введите полный путь к файлу (По умолчанию в конец добавляется .txt): ";
	...
	if (std::experimental::filesystem::is_regular_file(fileName) && file.good()) {
		if (FileCheck(fileName) == true) {
			...
			if (!file.eof()) {
				...
				string buf;
				if (!file.eof()) {
					file >> buf.assign((std::istreambuf_iterator<char>(file.rdbuf())), std::istreambuf_iterator<char>()); // записать весь буфер в файл
				}
